...
/*
* Planets Queries II
* Cases, both in tree, both in cycle, one in each
* con pista
*/
...
int cycle_dist(int a, int b) {
    int len = cycle_size[component[a]];
    return (idx_in_cycle[b] - idx_in_cycle[a] + len) % len;
}
 
int ans(int a, int b) {
	dump(a, b);
	dump(component[a], component[b])
    if (component[a] != component[b]) return -1;
 
    int da = distance_to_cycle[a];
    int db = distance_to_cycle[b];
	dump(da, db);
    ...
    if (db > 0) {
        ...
        int diff = da - db;
        int x = binary_lifting(a, diff);
 
        return (x == b ? diff : -1);
    }
 
    // b is in the cycle
    int a_cycle = binary_lifting(a, da);
 
    return da + cycle_dist(a_cycle, b);
}
...
void solve() {
    ...
    // Fill tree node distances/components.
    rep(i, n) {
        calc_distance(i);
    }
	
	dump(distance_to_cycle, idx_in_cycle, component);
