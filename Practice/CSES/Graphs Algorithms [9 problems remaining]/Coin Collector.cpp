...
void solve() {
	...
	vi new_graph(ans), dp(ans), indeg(ans);
	vvi nadj(ans);
	
	rep(i, n){
		new_graph[kingdom[i]] += coins[i];
		for (int u : adj[i]){
			if (kingdom[i] == kingdom[u]) continue;
			nadj[kingdom[i]].pb(kingdom[u]);
			indeg[kingdom[u]]++;
		}
	} 
	...
	while (!q.empty()){
		...
		for (int u : nadj[idx]){
			...
			dp[u] = max(dp[u], dp[idx] + new_graph[u]);
			if (indeg[u] == 0){
				q.push(u);
			}
		}
